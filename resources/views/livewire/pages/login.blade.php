<div>
    ...
    <main class="bg-transparent bg-cover bg-center bg-no-repeat text-white min-h-screen flex items-center justify-center font-['Poppins']" style="background-image: url('/storage/background-auth/image-auth.webp');">
        ...
        <div class="bg-black/45 backdrop-blur-sm rounded-[20px] p-14 w-full max-w-[450px] shadow-[0_10px_28px_rgba(0,0,0,0.25)]">
            <h1 class="text-[34px] tracking-wide mb-8 text-center font-semibold">Log In</h1>
            ...
            <form wire:submit.prevent="login">
                ...
                <div class="mb-6 relative" x-data="{ show: false }">
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="block text-white/90 text-sm font-medium">Password</label>
                        <a href="{{ route('forgot-password') }}" class="text-white/60 no-underline text-xs hover:text-[#fed136] transition-colors">
                            Forgot Password?
                        </a>
                    </div>
                    <div class="relative">
                ...
                <div class="flex items-center justify-between mt-8">
                    <button type="submit" class="bg-[#8B1A1A] text-white px-8 py-3 rounded-lg font-medium transition-all duration-300 ease-in-out cursor-pointer border-none hover:bg-[#a52323] hover:shadow-lg">
                        Log In
                    </button>
                    <a href="{{ route('signup') }}" class="text-white/70 no-underline text-sm hover:text-[#fed136] transition-colors">
